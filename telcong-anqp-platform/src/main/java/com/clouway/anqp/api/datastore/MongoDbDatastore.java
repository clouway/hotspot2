...
final class MongoDbDatastore implements Datastore {
  ...
  @Inject
  public MongoDbDatastore(Provider<MongoDatabase> database) {
  ...
  public <T> void ensureIndex(Class<T> entity, Index index) {
    ...
    database.get().getCollection(kindOf(entity)).createIndex(nativeIndex, new IndexOptions().background(true));
  }
  ...
  public void ensureIndex(String nativeName, Index index) {
    Document nativeIndex = new Document();
    index.apply(nativeIndex);
    database.get().getCollection(nativeName).createIndex(nativeIndex, new IndexOptions().background(true));
  }
  ...
  public <T> List<T> findAll(Class<T> clazz) {
    ...
    MongoCursor<Document> cursor = database.get().getCollection(entityKind).find().iterator();
  ...
  public <T> void delete(Class<T> clazz, Filter filter) {
    MongoQuery query = filter.build();
    query.deleteFrom(database.get().getCollection(kindOf(clazz)));
  }
  ...
  public <T> void deleteById(Class<T> clazz, Object id) {
    database.get().getCollection(kindOf(clazz)).deleteOne(new Document("_id", id));
  }

  @Override
  public <T> Long entityCount(Class<T> clazz, Filter filter) {
    return filter.build().countIn(database.get().getCollection(kindOf(clazz)));
  }

  @Override
  public <T> Long entityCount(Class<T> clazz) {
    return database.get().getCollection(kindOf(clazz)).count();
  }
  ...
  public <T> void upsert(T entity, Filter filter) {
    ...
    database.get().getCollection(kind).findOneAndReplace(updateId, object, new FindOneAndReplaceOptions().upsert(true));
  }

  @Override
  public <T> T findAndModify(Class<T> clazz, Filter filter, UpdateStatement statement) {
    Document entity = filter.build().findAndModify(database.get().getCollection(kindOf(clazz)), statement);
    if (entity == null) {
      return null;
    }
  ...
  public <T> void update(Class<T> clazz, Filter filter, UpdateStatement statement) {
    filter.build().update(database.get().getCollection(kindOf(clazz)), statement);
  }

  private <T> List<T> execute(Class<T> clazz, MongoQuery query) {
    String entityKind = kindOf(clazz);

    MongoCursor<Document> cursor = query.findIn(database.get().getCollection(entityKind));
  ...
  private <T> MongoCollection<Document> collection(T newEntity) {
    return database.get().getCollection(kindOf(newEntity.getClass()));
  }
